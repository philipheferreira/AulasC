...
#include<stdlib.h>
#include<locale.h> // essa biblioteca tem a função de adicionar a acentuação para poder usar em alguma linguagem especifica, quando eu adiciono coloco em aberto para aceitar todas as acentuações de todas as lingaguens
...
setlocale(LC_ALL, ""); // Comando para adicionar a acentuação no codigo
...
printf("\t\tOlaaaaaaaaaaaaaaaaaaaaaaaa"); // No final de cada comando em C devesse colocar ponto e virgula, para o computador, através do compilador, assimilar que aqui é o final da linha e o final desse comando, se não o codigo da erro 
printf("\n\n E aiiiiiiiiiiii????") // existem comandos que podesse usar para controlar o espaçamento plotado na nela com relação ao texto
printf("Agora eu tenho acento. Éééééééééééééé")// O comando \t da um tab na linha, e o comando \n da um espaço para baixo
